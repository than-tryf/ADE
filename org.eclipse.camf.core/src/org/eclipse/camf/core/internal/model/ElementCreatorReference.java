...
public class ElementCreatorReference {
  ...
  private static class SourceMatcher {
    ...
    SourceMatcher( final IConfigurationElement element )
      ...
      String srcclsatt = element.getAttribute( Extensions.CLOUD_ELEMENT_CREATOR_SOURCE_CLASS_ATTRIBUTE );
      ...
      if ( bundle != null ) {
        try {
          ...
          String idatt = ( ( IConfigurationElement ) element.getParent() ).getAttribute( Extensions.CLOUD_ELEMENT_CREATOR_ID_ATTRIBUTE );
      ...
      String srcptrnatt = element.getAttribute( Extensions.CLOUD_ELEMENT_CREATOR_SOURCE_PATTERN_ATTRIBUTE );
      if ( srcptrnatt != null ) {
        try {
      ...
      String srcprrtyatt = element.getAttribute( Extensions.CLOUD_ELEMENT_CREATOR_SOURCE_PRIORITY_ATTRIBUTE );
      if ( srcprrtyatt != null ) {
        try {
      ...
      String srcdfltatt = element.getAttribute( Extensions.CLOUD_ELEMENT_CREATOR_SOURCE_DEFAULT_ATTRIBUTE );
      if ( srcdfltatt != null ) {
      ...
      String srcmtchratt = element.getAttribute( Extensions.CLOUD_ELEMENT_CREATOR_SOURCE_MATCHER_ATTRIBUTE );
      if ( srcmtchratt != null ) {
        try {
          this.sourceMatcher
            = ( ICreatorSourceMatcher ) element.createExecutableExtension( Extensions.CLOUD_ELEMENT_CREATOR_SOURCE_MATCHER_ATTRIBUTE );
  ...
  private static class TargetMatcher {
    ...
    TargetMatcher( final IConfigurationElement element )
      ...
      // Get the base classes for the class loading
      String trgtclsatt = element.getAttribute( Extensions.CLOUD_ELEMENT_CREATOR_TARGET_CLASS_ATTRIBUTE );
      ...
      this.targetName = element.getAttribute( Extensions.CLOUD_ELEMENT_CREATOR_TARGET_NAME_ATTRIBUTE );
     ...
     */
    public boolean matches( final Class< ? extends ICloudElement > target ) {
   ...
   */
  private ICloudElementCreator elementCreator;
   ...
   */
  public boolean checkTarget( final Class< ? extends ICloudElement > target )
      throws ProblemException {
  ...
  /**
   * Get the {@link ICloudElementCreator} from the corresponding extension. If
   ...
   * <code>null</code>.
   * 
   * @return The associated {@link ICloudElementCreator} or <code>null</code> if
   * the creation of the creator failed.
   */
  public ICloudElementCreator getElementCreator() {
    
    if ( this.elementCreator == null ) {
      try {
        this.elementCreator
          = ( ICloudElementCreator ) this.configurationElement.createExecutableExtension( Extensions.CLOUD_ELEMENT_CREATOR_EXECUTABLE );
      } catch( CoreException cExc ) {
  ...
  public String toString() {
    ...
    + this.configurationElement.getAttribute( Extensions.CLOUD_ELEMENT_CREATOR_ID_ATTRIBUTE );
  }
  ...
  private void initSourceMatchers() throws ProblemException {
    ...
    IConfigurationElement[] children
      = this.configurationElement.getChildren( Extensions.CLOUD_ELEMENT_CREATOR_SOURCE_ELEMENT );
  ...
  private void initTargetMatcher() throws ProblemException {
    IConfigurationElement[] children
      = this.configurationElement.getChildren( Extensions.CLOUD_ELEMENT_CREATOR_TARGET_ELEMENT );
