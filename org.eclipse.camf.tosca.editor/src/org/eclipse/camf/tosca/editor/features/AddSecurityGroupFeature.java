...
public class AddSecurityGroupFeature extends AbstractAddShapeFeature {

  private static final IColorConstant E_CLASS_TEXT_FOREGROUND = IColorConstant.BLACK;
  private static final IColorConstant E_CLASS_FOREGROUND = new ColorConstant( 98,
                                                                              131,
                                                                              167 );
  private static final IColorConstant E_CLASS_BACKGROUND = new ColorConstant( 0,
                                                                              255,
                                                                              80 );

  public AddSecurityGroupFeature( final IFeatureProvider fp ) {
    super( fp );
  }
  ...
  public boolean canAdd( final IAddContext context ) {
        ...
        && !diagraminstance )
    {
      ...
      if (file.getResource().getFileExtension().equals( "sg" )){
  ...
  public PictogramElement add( final IAddContext context ) {
    ...
    if (context.getNewObject() instanceof ResourceCloudElement) {
      ...
      addedClass.setArtifactType( new QName( "SecurityGroup" ) );
    } else if( context.getNewObject() instanceof SecurityGroup ) {
      sg = (SecurityGroup) context.getNewObject();
      addedClass = ToscaFactory.eINSTANCE.createTDeploymentArtifact();
      addedClass.setName( sg.getUID() );
      addedClass.setArtifactType( new QName( "SecurityGroup" ) );
      ...
      CreateSecurityGroupFeature createSGFeature = new CreateSecurityGroupFeature( new ToscaFeatureProvider( getDiagramBehavior().getDiagramContainer()
        .getDiagramTypeProvider() ) );      
      createSGFeature.setContextObject( addedClass );
      if( createSGFeature.canCreate( createContext ) ) {
        createSGFeature.create( createContext );
      }
    ...
    RoundedRectangle roundedRectangle;
    {
      ...
      gaService.setLocationAndSize( roundedRectangle,
                                    0,
                                    StyleUtil.APP_COMPONENT_HEIGHT - 70,
                                    width,
                                    height );
    ...
    // SHAPE WITH TEXT
    {
      ...
      String name = sg != null ? sg.getName() : addedClass.getName();
